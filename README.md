...
Parse — extract clean, structured data out of messy real-world resume PDFs (personal info, education, experience, skills, certifications, projects).
Match — compare the candidate's resume against a job description using semantic similarity and embeddings, not just keyword overlap.
Evaluate — score the candidate across multiple weighted dimensions (skills, experience, education) to produce an interpretable evaluation report.
Guide — for candidates who fall short, generate a personalized, LLM-powered roadmap showing exactly what to learn to close the gap for that specific role.
Decide — surface all of this in a bulk-screening frontend where an HR reviewer can go through candidates and select or reject them directly.
...
Upload a job description (or paste the text in).
Upload one or more candidate resumes (PDF).
The app parses each resume, scores it against the job description, and displays a ranked, structured breakdown per candidate.
For candidates below the threshold, view their generated skill roadmap.
Select or reject each candidate — the app generates an appropriate message either way.
Screenshots
